...
@injectable
class EmployerLongTermAuthorizePaymentBloc extends Bloc<
    EmployerLongTermAuthorizePaymentEvent,
    EmployerLongTermAuthorizePaymentState> {
  final IMainFacade _iMainFacade;

  EmployerLongTermAuthorizePaymentBloc(this._iMainFacade)
      : super(const EmployerLongTermAuthorizePaymentState()) {
    on<EmployerLongTermAuthorizePaymentEvent>((event, emit) async {
      await event.map(
        onAuthorizePayment: (value) async {
          ...
          final response =
              await _iMainFacade.employerApplicantsAccept(id: event.id);

...
@injectable
class EmployerFullPositionDetailBloc extends Bloc<EmployerFullPositionDetailEvent, EmployerFullPositionDetailState> {
  final IMainFacade _iMainFacade;

  EmployerFullPositionDetailBloc(this._iMainFacade) : super(EmployerFullPositionDetailState.initial()) {
    on<EmployerFullPositionDetailEvent>((event, emit) async {
      await event.map(
        getPositionDetail: (value) async {
          emit(state.copyWith(isLoading: true));
          final response = await _iMainFacade.getEmployerPositionDetail(id: value.id, postType: 2);

...
@injectable
class EmployerLongTermDetailBloc extends Bloc<EmployerLongTermDetailEvent, EmployerLongTermDetailState> {
  final IMainFacade _iMainFacade;

  EmployerLongTermDetailBloc(this._iMainFacade) : super(EmployerLongTermDetailState.initial()) {
    on<EmployerLongTermDetailEvent>((event, emit) async {
      await event.map(
        getPositionDetail: (value) async {
          emit(state.copyWith(isLoading: true));
          final response = await _iMainFacade.getEmployerPositionDetail(id: value.id);
